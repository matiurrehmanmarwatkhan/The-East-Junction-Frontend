...
export default function ReservationsView({
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!formData.name || !formData.phone || !formData.date || !formData.time) {
      setStatus("error");
      setErrorMessage(
        "Please fill in all mandatory indicators (Name, Phone, and Date, and Time).",
      );
      return;
    }
    setStatus("booking");
    try {
      ...
      setErrorMessage(
        err.response?.data?.message ||
          err.message ||
          "Could not link connection to reservation engine. Check network.",
      );
    }
  };
  const timeSlots = [
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="font-serif text-3xl sm:text-5xl text-white font-bold mt-2.5">
            The Reservation{" "}
            <span className="text-gold-400 italic font-serif">Desk</span>
          </h2>
          <p className="text-stone-400 text-xs max-w-lg mx-auto mt-4 leading-relaxed">
            Reserve premium lounge dining, open balcony views, or custom
            birthday events. All bookings are registered directly and confirmed
            within minutes.
          </p>
        ...
        <div className="max-w-4xl mx-auto">
          <AnimatePresence mode="wait">
            {status !== "completed" ? (
              ...
              >
                {selectedPackage && (
                  <div className="mb-8 p-4 border border-gold-400/25 bg-gold-400/5 text-gold-350 text-xs flex items-center justify-between">
                    <span className="flex items-center">
                      <PartyPopper className="w-4 h-4 mr-2 text-gold-400 animate-bounce" />{" "}
                      Selected Event Suite:{" "}
                      <strong className="ml-1 text-gold-450">
                        {selectedPackage}
                      </strong>
                    </span>
                    <button
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-stone-400 font-semibold uppercase tracking-wider block">
                        Target Dinner Date{" "}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center space-x-4 bg-stone-950 border border-white/10 rounded-none p-1 max-w-[200px] h-[46px]">
                        ...
                        <span className="font-mono text-sm text-white font-bold text-center w-8">
                          {formData.guests}
                        </span>
                        <button
                          type="button"
                      ...
                      <p className="text-[10px] text-stone-500 font-mono">
                        Max group booking online: 60 guests.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-white/5">
                    {/* Full Name */}
                    <div className="space-y-2">
                      <label className="text-stone-400 font-semibold uppercase tracking-wider flex items-center">
                        <User className="w-3.5 h-3.5 mr-1 text-gold-400" /> Full
                    ...
                    <div className="space-y-2">
                      <label className="text-stone-400 font-semibold uppercase tracking-wider flex items-center">
                        ...
                        WhatsApp / Cell <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="tel"
                        name="phone"
                  ...
                  <div className="space-y-2 pt-4">
                    <label className="text-stone-400 font-semibold uppercase tracking-wider block">
                      Special requests / Allergies / Table custom layouts
                    </label>
                    <textarea
                      name="specialRequest"
                  ...
                  {/* Error reports */}
                  {status === "error" && (
                    <div className="p-4 border border-red-500/25 bg-red-950/20 text-red-400 font-medium rounded-none">
                      {errorMessage}
                    </div>
                  )}
              ...
              >
                ...
                <p className="text-[10px] font-mono tracking-[0.22em] text-gold-400 uppercase">
                  Sovereign Booking Verified
                </p>
                <h3 className="font-serif text-2xl text-white font-bold mt-2.5">
                  Diner Session Confirmed
                </h3>

                {/* Gold Crest code panel */}
                <div className="w-full border-t border-b border-dashed border-white/5 bg-stone-950 py-4.5 my-6">
                  <span className="text-stone-500 text-[9px] font-mono tracking-widest uppercase block mb-1">
                    Confirmation Voucher Code
                  </span>
                  <span className="font-mono text-xl text-gold-400 font-bold tracking-[0.16em] gold-glow">
                    {voucherId}
                  </span>
                </div>
                ...
                <div className="w-full text-left space-y-3.5 text-xs text-stone-300 pb-6 border-b border-white/5 mb-6">
                  <div className="flex justify-between">
                    ...
                    <strong className="text-white text-right font-serif">
                      {formData.name}
                    </strong>
                  </div>
                  <div className="flex justify-between">
                ...
                <div className="flex items-start space-x-2 text-stone-500 text-[10px] text-left leading-relaxed">
                  ...
                  <p>
                    Show this verified voucher receipt at{" "}
                    <strong>
                      The East Junction Spogmai Plaza, University Road Peshawar
                    </strong>{" "}
                    reception deck to settle your exclusive table group.
                  </p>
                </div>
