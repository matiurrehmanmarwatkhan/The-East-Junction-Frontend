...
export default function MenuView() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      console.error("Failed to fetch menu in MenuView: using luxury local seeds", err);
    }).finally(() => {
      setLoading(false);
    });
  }, []);
  const getCategoryIcon = (iconName) => {
    switch (iconName) {
      ...
      case "Soup":
        return <UtensilsCrossed className="w-4 h-4" style={{ transform: "rotate(18ddeg)" }} />;
      case "Wine":
  ...
  return <section id="interactive-menu-view" className="bg-stone-950 min-h-screen pt-32 pb-24 text-stone-300">
    ...
    /* Elite Banner Header */
        ...
        <div className="text-center mb-16">
          <p className="text-[10px] font-mono tracking-[0.24em] text-gold-400 uppercase">Sovereign Dining Selection</p>
          <h2 className="font-serif text-3xl sm:text-5xl text-white font-bold mt-2.5">
            The East Junction <span className="text-gold-400 italic font-serif">A La Carte</span>
          </h2>
          <p className="text-stone-400 text-xs max-w-lg mx-auto mt-4 leading-relaxed">
            Search our luxury dishes, choose specific filters, and order directly to your door at Spogmai Plaza via instant WhatsApp integration. One click, infinite pleasure.
          </p>
  ...
  >
    ...
    /* Live Grid of Menu Items */
        ...
        {loading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3, 4, 5, 6].map((i) => <div key={i} className="glass-panel h-[400px] rounded-none shimmer-active" />)}
          </div> : filteredItems.length > 0 ? <motion.div
    layout
  ...
  />
    ...
    /* Tags */
                    ...
                    {item.tags && item.tags.length > 0 && <div className="absolute top-3 left-3 flex flex-wrap gap-1">
                        {item.tags.map((tag, idx) => <span
    key={idx}
  ...
  >
          ...
          </motion.div> : <div className="text-center py-16 bg-stone-900/20 border border-stone-800/80 rounded-2xl max-w-lg mx-auto">
            <span className="text-3xl">🍽️</span>
            <h4 className="font-serif text-lg text-white font-semibold mt-4">No matching dishes discovered</h4>
            <p className="text-stone-400 text-xs mt-2">Try adjusting your search query, or choosing a different filter category tab above.</p>
            <button
    onClick={() => {
      setSearchQuery("");
